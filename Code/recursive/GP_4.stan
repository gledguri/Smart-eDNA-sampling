...
data {
  ...
  matrix[N_total,N_depths] des_mat;
  vector[N_total] d_idx;
  // Priors
...
model {
  ...
  matrix[N_total,N_depths] y_mat = y .* des_mat; //unfinished
  // Process each depth
  for (d in 1:N_depths) {
    ...
    for (i in 1:N_d) {//unfinished
      // X_d[i] = X[idx[i]];
      y_d[i] = y_mat[idx[i]];//unfinished
    }
    
    array[N_d] vector[2] X_d = X[start:end];
    // vector[N_d] y_d = y[start:end];
    vector[N_d] y_d = y_mat[d_idx[d],d];//unfinished
