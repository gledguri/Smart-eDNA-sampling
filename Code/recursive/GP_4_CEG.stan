...
functions {
  ...
  vector gp_predict_depth(int N,
                          ...
                          real mu) {
    matrix[N, N] K = gp_exp_quad_cov(X, alpha, rho);
    matrix[N, N_pred] k_pred = gp_exp_quad_cov(X, X_pred, alpha, rho);
    matrix[N, N] L_K;
    vector[N] K_div_y;
...
model {
  ...
  rho_sd ~ exponential(rho_sd_prior[1]);
  sigma ~ normal(sigma_prior[1], sigma_prior[2]);
  mag_rho ~ normal(mag_rho_prior[1],mag_rho_prior[2]);
  ...
  for (d in 1:N_depths) {
    ...
    gp_marginal_lp(X_d, y_d, alpha[d], rho*(10^mag_rho), sigma, mu[d]);
  }
}
...
generated quantities {
  ...
  for (d in 1:N_depths_pred) {
    ...
    y_pred[start_p:end_p] = gp_predict_depth(N_d, N_d_p, X_d, y_d, X_d_pred, alpha[d], rho*(10^mag_rho), sigma, mu[d]);
  }
}
